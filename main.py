...
@app.route("/stateclient", methods=["GET"])
def cstate():
    student_list = Student.query.all()
    return render_template("index_client.html", student_list=student_list)



#geochart
@app.route("/cpi/geo", methods=["GET"])
def geo():
    return render_template("geochart india.html")
...
#home page admin
@app.route("/admincpi", methods=["GET"])
def admincpi():
    return render_template("admincpi.html")
...
def add_student():
    if request.method == "GET":
        ...
        if Student.query.filter_by(roll_number=roll).first() == None:
            ...
            data = Student(roll_number=roll, first_name=f_name, last_name=l_name)
...
def update_student(student_id):
    try:
        ...
        if request.method == "GET":
            ...
            c_id = updated_course
            s_id = student_id
            e_data = Enrollments(estudent_id=s_id, ecourse_id=c_id)
...
def view_student(student_id):
    ...
    for enrollment in enrollments:
        ...
        c_desc = Course.query.filter_by(course_id=c_id).first().course_description
...
def delete_enrollment(student_id, course_id):
    try:
        ...
        for data in Enrollments.query.filter_by(
...
def view_courses():
    ...
    return render_template("view_courses.html", course_list=course_list)
...
def add_course():
    try:
        if request.method == "GET":
            ...
            data = Course(course_code=code, course_name=c_name, course_description=desc)

            try:
                db.session.add(data)
            except:
                db.session.rollback()
                return render_template("exists_2.html")
            else:
                db.session.commit()
    except:
        return render_template("exists_2.html")
...
def update_course(course_id):
    try:
        ...
        if request.method == "GET":
            return render_template("update_course.html", data=data, course_id=course_id)
        elif request.method == "POST":
            c_name = request.form.get("c_name")
            desc = request.form.get("desc")
            ...
            student_data.append(
                {
                    ...
                    "f_name": students.first_name,
                    "l_name": students.last_name,
                }
            )

        return render_template(
            "view_course.html", student_data=student_data, course_data=course_data
        )
...
def login():
    if request.method == "GET":
        ...
        if uname == "host" and password == "cpi":
            return redirect("/admincpi")
        else:
            return render_template("cpi.html")
